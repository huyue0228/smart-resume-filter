...
import { Badge, Button, Card, Drawer, Empty, Progress, Space, Tag, Typography } from 'antd'
import { SyncOutlined } from '@ant-design/icons'
import { fetchPipelineRuns } from '../api/services'

const ACTIVE_STATUSES = new Set(['pending', 'running', 'waiting_conflict'])
const STATUS_META = {
...
function TaskCard({ run, compact = false }) {
  ...
  return (
    <Card size="small" style={{ marginBottom: compact ? 0 : 12 }}>
      <Space direction="vertical" size={6} style={{ width: '100%' }}>
        ...
        {!compact && (run.failed_count || run.review_count || run.dispatch_count || run.archive_count) ? (
          <Typography.Text type="secondary">
            成功 {run.success_count || 0} · 失败 {run.failed_count || 0} · 待复核 {run.review_count || 0} · 待下发 {run.dispatch_count || 0} · 归档 {run.archive_count || 0}
          </Typography.Text>
        ) : null}
        {!compact && run.error ? <Typography.Text type="danger">{run.error}</Typography.Text> : null}
      </Space>
    </Card>
  )
}
...
export default function ProcessingTaskCenter() {
  ...
  return (
    <>
      {activeRuns[0] ? (
        <div style={{ position: 'fixed', right: 24, bottom: 24, width: 340, zIndex: 20 }}>
          <TaskCard run={activeRuns[0]} compact />
        </div>
      ) : null}
      ...
      >
        {visibleRuns.length ? visibleRuns.map((run) => <TaskCard key={run.id} run={run} />) : <Empty description="暂无处理任务" />}
      </Drawer>
    </>
  )
}

...
| Config                | 系统配置项，保存 AI 阈值、超时、WeLink 开关及由 `settings.manage_ai_connection` 权限维护的加密 AI 连接配置 | 系统 / 配置   |
...
- 简历库“处理简历”按请求 `modes` 多选 Rule / AI 重跑时，每个模式拥有独立 `ProcessingRun` 与 `mode`；两种来源自动尝试必须并存，顺序执行和重跑均不得互相取消。手动尝试、已反馈尝试和已通过流程不受影响。规则分配、AI分配子页仅按固定 `source` 浏览已有尝试，不触发重跑或提供处理模式选择；两页从同一 `AssignmentAttempt.match_reason` 字段展示分配原因。
...
> `Config` 保存普通运行参数，以及由 `settings.manage_ai_connection` 权限维护的 AI 模型连接配置。模型连接仅从这些数据库记录读取，未保存 API Key 时 AI 未启用；运行时不读取任何环境变量或部署文件中的模型连接信息。`backend/config/ai_models.json` 只为系统设置表单提供 `api_style`、模型和 Base URL 模板。连接项为 profile、`api_style`、`model_name`、`base_url` 和 API Key。API Key 使用 Django `SECRET_KEY` 派生的 Fernet 密钥加密后写入 `Config.value`，只在服务端调用时解密；任何 GET、序列化、审计、任务结果或错误响应都不返回明文或密文，仅返回是否已配置等安全摘要。

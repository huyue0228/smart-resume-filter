...
> `block_reason/block_detail` 只保留给其它可恢复的非终态阻塞；生成有效分配尝试、手动强制分配、流程通过、流程归档或简历库重新处理重开时，应清空这两个字段。
...
> 状态约束：反馈一旦提交即锁定，不允许二次修改。Rule 创建 `source=rule`，普通 AI 与后台专项路由创建 `source=ai`，人工强制分配创建 `source=manual`。Rule/普通 AI 自动生成 `pending_review / pending_dispatch` 时必须在同一短事务占用一条任务容量并关联 `capacity_reservation`；下发前取消可幂等释放，进入 `dispatched` 后不再释放。后台专项路由额外写 `route_code=ai_special_route`，以两段系统部门事件直达三级。
...
| event_type       | varchar               | attempt_created / review_confirmed / department_dispatched / department_transferred / feedback_passed / feedback_rejected / cancelled |
...
> 候选人删除必须先做服务层影响检查：存在 `AssignmentAttempt`、任何反馈、`AgentDispatchDecision` 或 `AssignmentHandlingEvent` 时返回 409，不执行数据库级联。上述 Candidate 级 CASCADE 只用于完全没有受保护历史的数据维护场景。
...
> ScopeItem 覆盖整个候选人处理链而不是某一个步骤。Step2 对规则不通过项直接写 completed；Step3 对业务主数据缺口写 completed，对通过者写固定引用；Step4 只领取仍 pending 的项。公开 `reason_code` 至少包含 `education_not_eligible / school_not_eligible / job_not_found / secondary_department_missing / major_not_matched / job_responsibility_missing / llm_timeout / resume_text_unavailable / ai_connection_error / ai_rate_limited / ai_invalid_output / ai_reference_invalidated / ai_dispatched / ai_review / ai_archived`；其中 `major_not_matched` 只允许由 Rule 专业审核写入。旧专项原因和缺接口人原因不兼容、不迁移。

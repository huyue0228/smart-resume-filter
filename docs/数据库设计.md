...
> 默认院校标签由用户提供。院校分类时，学校命中 `School.school_tag_id` 则写入候选人学历标签；未命中学校或学校未配置标签时，优先使用 `is_default=true` 的标签；没有默认标签但存在启用且名称或编码为“非目标院校”的标签时，应用层将其作为非目标院校兜底标签。
...
| status                   | varchar   | pending / running / waiting_conflict / success / partial_failed / failed / undone |
...
| status | varchar | pending / running / waiting_conflict / success / partial_failed / failed / skipped |
...
> `Config` 只保存可由配置页维护的非敏感运行参数。大模型 profile 由 `backend/config/ai_models.json` 声明，`apps.pipeline.ai_config` 加载 profile 并应用部署环境覆盖；真实 API Key 只从 profile 指定的环境变量读取，不落库、不经普通配置 API 返回。
> `Config.key` 必须来自后端注册表白名单，注册表同时声明值类型、范围、默认值和所需权限；未知 key 不创建。业务规则和 AI 非敏感运行参数可授予 HR/管理员维护，`processing_run_heartbeat_timeout_seconds`、认证与安全配置仅管理员维护。

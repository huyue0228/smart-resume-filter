...
6. **系统简历状态为推导态**：简历库展示的待处理、已分类、已分配、待筛选、筛选通过、筛选不通过由 CandidateWorkflow、AssignmentAttempt、当前 Resume 分类字段和院校标签推导，不覆盖导入原始 `Resume.status`。
...
10. **招聘周期边界**：不引入批次/周期表，以当前数据池作为一个持续招聘周期；同一 `Candidate` 只维护一个当前分配流程。历史招聘周期隔离暂不设计。
...
| Config                | 系统配置项，保存 AI 阈值、超时、WeLink 开关及管理员维护的加密 AI 连接配置                                  | 系统 / 配置   |
...
| status                        | varchar                   | 导入文件中的原始应聘状态（待处理等），可作为原始字段展示/筛选；不作为简历库“系统简历状态” |
...
> 流程口径：一个候选人业务上只维护一个当前分配流程，`CandidateWorkflow.candidate_id` 已由 `OneToOneField` 施加数据库唯一约束，`revision` 字段已落地。当前自动分配已在同一事务内以 `select_for_update` 锁定 Candidate 与 CandidateWorkflow；所有人工流程写操作统一采用同一锁与版本语义仍为**目标迁移（未实现）**。归档候选人不单独建表，通过 `status=archived` + 归档字段查询；后续规则调整不会自动恢复归档流程，HR 可手动强制分配，或在简历库“处理简历”入口按弹窗选择的系统简历状态范围主动重开。
...
> 系统简历状态推导：`raw=待处理`、`classified=已分类`、`allocated=已分配`、`pending_screening=待筛选`、`screening_passed=筛选通过`、`screening_rejected=筛选不通过`。该状态不单独落库；查询筛选时由应用层基于当前候选人/投递、分类字段、院校标签、最新有效 `AssignmentAttempt` 和 `CandidateWorkflow` 推导。多条历史并存时优先级为 `screening_passed > pending_screening > allocated > screening_rejected > classified > raw`；历史 rejected/cancelled 不覆盖下一志愿活动尝试。HR 主动“处理简历”时，`ProcessingRun.scope.system_statuses` 记录本次选择的状态范围。
...
| 状态        | 中文含义 | 进入方式                                          | 是否终态               |
| ----------- | -------- | ------------------------------------------------- | ---------------------- |
| pending     | 待分配   | 创建流程但尚未生成有效尝试；简历库按流程状态筛选 `pending` 时，也包含尚未创建 `CandidateWorkflow` 的候选人 | 否                     |
...
> `archive_reason` 存枚举，`archive_detail` 存人类可读说明和具体上下文，简历库按这两个字段展示归档原因。AI 建议归档、低于复核阈值、简历缺失/解析失败或最终失败且不生成 `AssignmentAttempt` 时，流程写入 `status=archived`、`archive_reason=agent_no_recommendation`，对应 `AgentDispatchDecision` 保留供 HR 重试、切 Rule 后重新处理或手动分配。`contact_not_found` 作为历史兼容枚举保留；当前志愿已匹配岗位和二级部门但缺可用二级接口人时，应使用 `block_reason=contact_not_found` / `block_detail`，流程保持 `in_progress`，不写入归档原因。
...
- AI 决策未生成尝试时，不新增“待人工”尝试状态；`CandidateWorkflow` 统一归档为 `agent_no_recommendation`，人工处置通过重试、切 Rule 后重处理或手动强制分配重新打开流程。
...
- 简历库“处理简历”按请求 `modes` 多选 Rule / AI 重跑时，每个模式拥有独立 `ProcessingRun` 与 `mode`；两种来源自动尝试必须并存，顺序执行和重跑均不得互相取消。手动尝试、已反馈尝试和已通过流程不受影响。规则分配、AI分配子页仅按 `source` 浏览已有尝试，不触发重跑。
- 简历库“处理简历”是 HR 主动重新分配入口，只处理 `ProcessingRun.scope.system_statuses` 和当前筛选条件命中的候选人，且必须携带至少一个 `modes` 项。该入口可重开已通过、已归档或已不通过候选人流程：保留历史尝试、反馈和审计记录，清空 `passed_attempt_id`、归档原因、阻塞原因和完成时间，将 `CandidateWorkflow.status` 恢复为 `in_progress`，再按所选 Rule / AI 分别生成新的自动尝试。AI 未启用时含 `ai` 的请求被拒绝，不降级为 Rule。
...
| CandidateWorkflow     | status                                     | 简历库流程状态筛选               |
...
> 步骤映射：`step1`=查重与志愿排序，`step2`=简历分类、分配与下发，`step3`=院校分类，`step4`=需求数据准备核对，`step5`=兼容旧入口的分配步骤别名。`all` 按 `step3 → step4 → step1 → step2` 执行，不额外执行 `step5`。上传含候选人时固定创建 Rule 运行，且仅 AI 已启用时创建额外 AI 运行；两个 `ProcessingRun` 独立记录、顺序先 Rule 后 AI 执行，各自按 `Step1 → Step2` 编排，绝不复用 `all`。简历库“处理简历”按钮按 `modes` 为每种已选模式创建独立、只含 `step2` 的运行，并携带 `scope.system_statuses`；仅请求缺失 `modes` 时才兼容单数 `mode`，空数组或非数组拒绝。生产异步提交返回 202，本地 `CELERY_TASK_ALWAYS_EAGER=True` 同步完成后返回 200，两者均返回独立运行列表与单数兼容项。提交时将命中的候选人写入 `ProcessingRunScopeItem`，后续执行只读取固化范围。顶栏任务 Badge 与点击后的非遮罩侧边 Drawer 轮询这些运行及阶段记录。父运行只汇总状态、阶段指针和最终业务结果计数；页面连续进度由各 `ProcessingRunStage` 的累计工作量计算。`pending` 收到取消请求后立即写为 `cancelled`；`running` / `waiting_conflict` 写为 `cancelling`，执行器在候选人或阶段边界停止、保留已落库结果并写入 `cancelled_at`。任务中心活跃查询包含 `cancelling`。
...
| ----- | ------- | ------------------------------------------------------------------------------------------------- |
| id    | PK      |                                                                                                   |
| key   | varchar | 如 ai_timeout_seconds、ai_dispatch_threshold、ai_review_threshold、ai_concurrency、ai_retry_count、ai_retry_backoff_seconds、processing_run_heartbeat_timeout_seconds、welink_enabled；应用层拦截重复 |
| value | jsonb   | 配置值                                                                                            |

> `Config` 保存普通运行参数，以及仅由管理员维护的 AI 模型连接配置。只要保存任一管理员连接项，即完整使用数据库连接且不读取任何环境变量；即使之后清除已保存 API Key，也必须视为该管理员连接未配置，不得回退到旧环境变量。profile 注册表仍位于 `backend/config/ai_models.json`，只在从未保存管理员连接项时提供 `api_style`、模型和 Base URL 默认模板及环境变量兼容兜底。管理员连接项为 profile、`api_style`、`model_name`、`base_url` 和 API Key。API Key 使用 Django `SECRET_KEY` 派生的 Fernet 密钥加密后写入 `Config.value`，只在服务端调用时解密；任何 GET、序列化、审计、任务结果或错误响应都不返回明文或密文，仅返回是否已配置等安全摘要。
> `Config.key` 必须来自后端注册表白名单，注册表同时声明值类型、范围、默认值和所需权限；未知 key 不创建。业务规则和 AI 非敏感运行参数可授予 HR/管理员维护；AI 连接配置、`processing_run_heartbeat_timeout_seconds`、认证与安全配置仅 `settings.manage_permissions` 或相应管理员权限可维护。AI 连接不经普通 `/api/configs/` 暴露，而使用专用管理员 API。
...
| Permission     | Django`auth_permission` + 后端权限注册表 | 权限点，采用稳定权限码，如`resume.view`、`resume.import`、`attempt.dispatch`、`attempt.review`、`attempt.assign_sub_contact`、`attempt.feedback`、`settings.manage_config`、`settings.manage_permissions`、`settings.manage_security` |

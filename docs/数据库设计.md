...
16. **岗位 Replace 使用现有模型全量同步**：固定业务键为招聘主体、二级部门、对外发布名称、内部职位名称、岗位类别，统一按去空白、忽略大小写后的值比较；唯一命中覆盖 `Job` 全部可导入属性和完整 `JobMajor` 明细并重新启用，未命中新建，文件外岗位只停用。文件或数据库重复键整批报错回滚；全无效文件不改变现有岗位。该语义复用 `Job.is_active`、`JobMajor` 和既有事务，不新增字段、约束或数据库迁移，也不删除历史 `ProcessingRunJobCapacity`。
...
> 岗位需求作为持续维护的主数据，不引入招聘周期表。岗位 Replace 的业务键固定为“招聘主体 + 二级部门 + 对外发布名称 + 内部职位名称 + 岗位类别”，各部分统一去空白并忽略大小写，且启用/停用岗位都参与查重和匹配。文件或数据库任一侧存在重复键时事务整体失败；唯一命中时覆盖全部可导入属性、`headcount`（外部列名仍为“需求数量”）和完整 `JobMajor` 集合并重新启用，未命中新建，文件外岗位只写 `is_active=false`。自动处理先在当前投递招聘主体内用对外职位名称精确映射唯一非空 `position_name`，再查询同主体、同内部职位名称的全部启用岗位；该映射是 Rule / AI 共享的独立服务，不读取候选人专业。精确映射行全部缺内部名称时记录 `internal_position_name_missing`，空值与非空值混合、多个内部职位或主体映射不唯一时记录 `job_mapping_ambiguous`，不再用包含匹配猜测。Rule 在映射后才读取 `MajorCategory / MajorAlias / JobMajor` 执行词表和名称包含审核；AI 不读取这些表做确定性专业审核，正式任务、直接处理、重试和结果落库均不得产生 `major_not_matched`。导入文件中工作职责为空的有效岗位行跳过；新增/编辑 API 要求工作职责非空。Replace 只有在文件至少产生一条有效岗位时才执行更新和停用，全无效文件不得改变现有岗位。数据库默认空字符串仅用于兼容历史岗位，Rule 可继续处理；AI 遇到历史空值时在 PDF 抽取和模型调用前写 `needs_attention + job_responsibility_missing`。岗位需求页面的“删除”采用 `is_active=false` 失效语义，不做硬删除。分配时只匹配启用岗位；历史投递、分配尝试和任务容量快照通过 `job_id` 与快照字段保持可追溯。本次同步和策略边界重构不改变现有导入接口或数据库 schema。
...
| Job               | 规范化招聘主体 + 二级部门 + 对外发布名称 + 内部职位名称 + 岗位类别；文件或数据库重复键整批拒绝 | 现有普通索引与应用层查询；不新增业务唯一约束 |
...
>
> 可编辑模块之外的最小安全底座、动态 JSON 字段载荷模板、省份白名单和 Pydantic JSON Schema/结构化输出协议不落在 `modules` 中，由后端 harness 固定生成；模型连接测试探针 `Reply with OK.` 同样由代码固定，不写入 `AIPromptVersion`，不参与草稿测试、发布或历史恢复。授权管理员仍可通过 `GET /api/ai-prompts/` 的 `full_prompt_preview` 查看上述固定 Prompt 全文和字段完整的占位载荷模板；这些可见性数据在请求时生成，不新增数据库字段，也不读取或持久化真实简历、岗位、候选人或院校数据。`ProcessingRun.prompt_version` 和院校省份任务参数保存版本字符串而非外键，使已创建任务在该版本转为 archived 后仍可继续读取，不受后来发布影响。
...
| 字段            | 类型             | 说明               |
| --------------- | ---------------- | ------------------ |
| ...AbstractUser |                  | 用户名/密码等 Django 标准账号字段；内置管理员调用 `set_unusable_password()` |

...
- 简历库“处理简历”按所选 Rule / AI 策略重跑时，只取消并重建未反馈的自动尝试；Rule 尝试与 AI 尝试都算自动尝试，手动尝试、已反馈尝试和已通过流程不受影响。规则分配、AI分配子页仅按 `source` 浏览已有尝试，不触发重跑。
- 简历库“处理简历”是 HR 主动重新分配入口，只处理 `ProcessingRun.scope.system_statuses` 和当前筛选条件命中的候选人。该入口可重开已通过、已归档或已不通过候选人流程：保留历史尝试、反馈和审计记录，取消未反馈自动尝试，清空 `passed_attempt_id`、归档原因、阻塞原因和完成时间，将 `CandidateWorkflow.status` 恢复为 `in_progress`，再按当前 Rule / AI 策略生成新的自动尝试。
...
> 步骤映射：`step1`=查重与志愿排序，`step2`=简历分类、分配与下发，`step3`=院校分类，`step4`=需求数据准备核对，`step5`=兼容旧入口的分配步骤别名。`all` 按 `step3 → step4 → step1 → step2` 执行，不额外执行 `step5`；普通简历上传由一个父运行编排 Step1/Step2，绝不复用 `all`。简历库“处理简历”按钮只触发带 `scope.system_statuses` 的 `step2`。提交时将命中的候选人写入 `ProcessingRunScopeItem`，后续执行只读取固化范围。父运行只汇总状态、阶段指针和最终业务结果计数；页面连续进度由各 `ProcessingRunStage` 的累计工作量计算。`pending` 收到取消请求后立即写为 `cancelled`；`running` / `waiting_conflict` 写为 `cancelling`，执行器在候选人或阶段边界停止、保留已落库结果并写入 `cancelled_at`。任务中心活跃查询包含 `cancelling`。

...
> AI 策略依赖 `ResumeProfile`。输入简历统一为 PDF；若简历文件缺失、PDF 解析失败、正文抽取失败或项目经历不足，AI 决策必须显式记录风险点，不得静默生成高置信度下发结论。数据库保存抽取后的 `raw_text` 和结构化 `ResumeProfile`，不保存 LLM 完整原始响应。写入 `ResumeProfile` 前，应用层移除 PDF 抽取正文 `raw_text` 中的 NUL（`0x00`）字符，并对模型结构化画像输出中的字符串递归执行相同清洗，覆盖画像文本字段及 JSONField 内任意层级的字符串；除 NUL 外不改变其它字符，避免 PostgreSQL text/jsonb 拒绝写入。该处理不新增字段或迁移。`file_checksum + profile_version` 用于判断是否复用画像，简历文件或画像版本变化后重新解析。

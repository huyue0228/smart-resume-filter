...
12. **招聘分析不建统计表**：v1 不增加数仓、日报快照或预聚合表，直接基于 `Resume.imported_at` cohort 和现有流程、尝试、AI 决策聚合；一级部门筛选和排行复用 `AssignmentAttempt.department_id → Department.parent_id` 的当前层级关系，二级部门排行继续使用已有部门名称快照。接口缓存 5 分钟，数据量扩大后再评估预聚合。Dashboard 下钻只在 URL 携带已生效筛选、数据项键和纯展示标题/范围文案，服务端只使用前两者实时复用同一查询，不保存报表连接、下钻会话或候选人快照；本次能力不新增模型、字段、索引或数据库迁移。
...
14. **导出配置不落库**：简历导出的字段目录是后端代码白名单，用户最近一次字段选择按用户保存在浏览器 `localStorage`，不新增用户偏好、导出任务或报表表；“同时下载简历原件”每次打开默认为 false，也不持久化。字段目录版本 2 的分组为“状态与原因”并删除 `processing_result`；历史选择由前端过滤失效键。Excel 和可选 ZIP 在请求内从 Candidate、Resume、Job、CandidateWorkflow、AssignmentAttempt、ProcessingRunScopeItem 等现有字段派生。
...
招聘分析沿用 `Resume(imported_at)` cohort 索引和外键索引，并新增 `AssignmentAttempt(created_at, source)`、`AssignmentAttempt(dispatched_at)`、`AssignmentAttempt(feedback_at)`、`AgentDispatchDecision(created_at, error_code)` 普通索引，用于事件趋势、来源和错误分布。投递数按 Resume 计数，其余阶段按 Candidate 去重；岗位优先读取 `CandidateWorkflow.current_resume`。每条 cohort 投递先按 `attempt_no / id` 选择最新非取消 `AssignmentAttempt`：一级部门筛选和 `primary_department_ranking` 通过该尝试当前 `department_id` 的 `Department.parent_id` 推导，二级部门筛选使用当前 `department_id`，既有 `department_ranking` 展示继续优先读取 `department_name_snapshot`。分析接口只读聚合并缓存 5 分钟，统一返回 `data_as_of`，不写回统计结果，也不改变 Candidate、Resume、CandidateWorkflow 或 AssignmentAttempt 状态机。

Dashboard 一/二级部门筛选、两级部门排行和结果报表可选部门过滤复用 `Resume.imported_at`、`CandidateWorkflow.current_resume_id`、`AssignmentAttempt.resume_id/status/attempt_no/id/department_id`、`Department.parent_id/level` 的现有字段与外键索引，通过子查询排除 `cancelled`；结果报表仍限制当前志愿，并从其最新非取消尝试的当前部门父级推导一级部门。`filter_options.primary_departments` 和二级部门选项的 `parent_id` 均由现有部门树生成，不落库为新字段。组合导出复用候选人、投递和尝试的既有查询路径；本次看板改动无模型或迁移变更。HC 配额新增的表、外键、检查/唯一约束和索引仍按既有迁移落地。

看板下钻复用上述同一 cohort 查询，只把 `analytics_*` 条件与 Candidate 主列表、组合导出或处理任务提交范围求交集，不写入统计表。分类、阶段、来源、AI 决策、岗位、部门、院校、学历和原因均从现有外键/状态/快照字段派生；二级部门用 `department_id + department_name_snapshot` 保持点击项口径，未通过自由文本只在前后端之间传递 SHA-256 派生键且不新增哈希列。下钻条件进入 `ProcessingRun.scope` 后仍由提交阶段固化到既有 `ProcessingRunScopeItem`，不建立新的报表连接表。

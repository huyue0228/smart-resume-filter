...
> 系统简历状态推导：`raw=待处理`、`classified=已分类`、`allocated=已分配`、`pending_screening=待筛选`、`screening_passed=筛选通过`、`screening_rejected=筛选不通过`。该状态不单独落库；查询筛选时由应用层基于当前候选人/投递、分类字段、院校标签、最新有效 `AssignmentAttempt` 和 `CandidateWorkflow` 推导。HR 主动“处理简历”时，`ProcessingRun.scope.system_statuses` 记录本次选择的状态范围。
...
> `archive_reason` 存枚举，`archive_detail` 存人类可读说明和具体上下文，简历库按这两个字段展示归档原因。`contact_not_found` 作为历史兼容枚举保留；当前志愿已匹配岗位和二级部门但缺可用二级接口人时，应使用 `block_reason=contact_not_found` / `block_detail`，流程保持 `in_progress`，不写入归档原因。
...
> 步骤映射：`step1`=查重与志愿排序，`step2`=简历分类、分配与下发，`step3`=院校分类，`step4`=需求录入，`step5`=兼容旧入口的分配步骤别名。`all` 执行正式步骤，顺序以后端设计为准，不额外执行 `step5`。简历库“处理简历”按钮只触发带 `scope.system_statuses` 的 `step2`，用于对选定系统简历状态范围内的候选人重新分配。

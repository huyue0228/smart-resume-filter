...
7. **招聘主体实体化**：`招聘主体` 不是普通文本字段，拆为 `RecruitingEntity` 主数据。投递、岗位需求、部门等只保存 `recruiting_entity_id`，以稳定主键关联主体；`code`、`name`、`display_name` 可随业务维护，便于未来支持 GW/YLS 等主体编码调整、启停、权限范围和主体合并/更名。
8. **业务唯一性不依赖数据库 unique**：除自增主键外，尽量不在数据库层做业务 `unique` 约束。导入、创建、编辑时由服务层按业务键查询并拦截重复；数据库只保留普通索引用于性能。这样更方便处理历史脏数据、人工合并、主体更名和导入回滚。
...
| 字段类型                                          | 建模口径                                        | 示例                                                                                |
| ------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------- |
| 会被多个表引用、需要启停/更名/权限/统计的业务对象 | 拆独立表 + FK                                   | 招聘主体`RecruitingEntity`、部门 `Department`、接口人 `Contact`、岗位 `Job` |
...
classDiagram
    ...
    RecruitingEntity "0..1" <-- "0..*" Department : scopes
...
| 对象                  | 含义                                                                                                       | 所属域        |
| --------------------- | ---------------------------------------------------------------------------------------------------------- | ------------- |
| RecruitingEntity      | 招聘主体主数据，如 GW/YLS；承载主体编码、展示名、启停状态和未来权限范围                                    | 主数据        |
...
| ------------ | -------- | ------------------------------------- |
| id           | PK       |                                       |
| code         | varchar  | 当前主体编码，如 GW / YLS；可随业务维护，应用层拦截重复 |
| name         | varchar  | 主体名称                              |
| display_name | varchar  | 页面展示名，可与 name 一致            |
| is_active    | bool     | 是否启用                              |
| created_at   | datetime |                                       |
| updated_at   | datetime |                                       |

> 招聘主体从字符串字段拆出，是为了支持主体编码/名称变更、启停、权限范围、统计口径和未来多主体扩展。`RecruitingEntity.id` 是各业务表外键引用的稳定标识；`code`、`name`、`display_name` 是可维护属性。导入时按当前 `code` 或 `name` 查询已有主体；查不到则创建或进入人工映射，不依赖数据库唯一约束。主体更名或改编码时只更新主表，不批量改历史投递、岗位和分配记录。
...
| recruiting_entity_id | FK→RecruitingEntity null | 所属招聘主体；主体无关的共享部门可为空 |
...
| -------------------- | -------------------- | ---------------------------------------------- |
| id                   | PK                   |                                                |
| recruiting_entity_id | FK→RecruitingEntity | 招聘主体                                       |
...
| valid_from           | date null            | 生效日期；为空表示不限制开始时间               |
| valid_to             | date null            | 失效日期；为空表示长期有效                     |
| remark               | text null            | 岗位需求调整说明                               |
| created_at           | datetime             |                                                |
| updated_at           | datetime             |                                                |

> 岗位需求作为持续维护的主数据，不引入招聘周期表。导入或编辑时按招聘主体、部门、岗位名称/对外名称、岗位类别等业务键在应用层匹配已有 `Job`；命中则更新，不再招聘的岗位通过 `is_active=false` 或 `valid_to` 失效，不做硬删除。分配时只匹配启用且在有效期内的岗位；历史投递和分配尝试通过 `job_id` 与快照字段保持可追溯。
...
| candidate_match_key     | varchar null | 可选匹配键，如`sha256(normalized_name + normalized_phone)`；非唯一，仅作查询加速/兼容 |
...
> 归并口径：导入服务优先按 `normalized_name + normalized_phone` 查询候选人；手机号缺失的候选人直接舍弃，不入库、不进入分配，并在导入结果中记录舍弃原因；命中一条候选人时复用，命中多条时阻断相关记录并进入导入错误清单，不在数据库层用 `unique` 强压。`candidate_match_key` 不是必要字段，更不能作为不可变身份真相。
...
| assigned_recruiting_entity_id | FK→RecruitingEntity null | 查重与志愿排序：分配主体                                                      |
...
> AI 策略依赖 `ResumeProfile`。输入简历统一为 PDF；若简历文件缺失、PDF 解析失败、正文抽取失败或项目经历不足，AI 决策必须显式记录风险点，不得静默生成高置信度下发结论。数据库保存抽取后的 `raw_text` 和结构化 `ResumeProfile`，不保存 LLM 完整原始响应。
...
| raw_response_summary      | text null              | 响应摘要；不保存 LLM 完整原始响应              |
| created_at                | datetime               |                                                |

> 默认置信度阈值为 `0.75` / `0.5`：`confidence_score >= 0.75` 且 `recommendation=dispatch` 时，创建 `source=ai, pending_dispatch` 的待下发尝试；`0.5 <= confidence_score < 0.75` 或 `recommendation=review` 时，创建 `source=ai, pending_review` 的 HR 复核项，不直接下发给接口人，HR 确认后再转为 `pending_dispatch`；`confidence_score < 0.5`、简历缺失、PDF 解析失败或 `recommendation=archive` 时可归档或人工处理，并记录 `agent_no_recommendation` 与风险点。AI 超时或调用失败时，不自动回退 Rule，应记录失败原因，由 HR 选择重试 AI 或切换 Rule。AI 决策不得绕过 `SchoolTagRule`、志愿顺序、反馈锁定和已通过候选人不可再分配等硬规则。表名保留 Agent，是因为 AI 策略内部以智能体方式完成简历阅读、岗位适配和分流建议。
...
- `source=ai` 时，`agent_decision_id` 必须非空，并保存 `confidence_score` 与 `review_required`；`0.5 <= confidence_score < 0.75` 的尝试进入 `pending_review`，HR 确认后转为 `pending_dispatch`。
...
| AI 策略生成尝试      | 生成`AgentDispatchDecision`；`confidence_score >= 0.75` 创建 `source=ai, pending_dispatch`，`0.5 <= confidence_score < 0.75` 创建 `source=ai, pending_review` 供 HR 复核，`confidence_score < 0.5` 或解析失败则建议归档或人工处理 |
| HR 确认 AI 复核项    | `pending_review → pending_dispatch`，保留 `review_required=true`、AI 理由、证据和风险点；后续按普通 HR 下发流转 |
...
| 模型              | 应用层拦截口径                                       | 数据库策略 |
| ----------------- | ---------------------------------------------------- | ---------- |
| RecruitingEntity  | code 或规范化 name 命中已有主体                      | 普通索引   |
...
| RecruitingEntity → Department                        | SET_NULL | 主体停用/删除不应删除部门树              |
...
> 步骤映射：`step1`=查重与志愿排序，`step2`=简历分类、分配与下发，`step3`=院校分类，`step4`=需求录入，`step5`=兼容旧入口的分配步骤别名。`all` 表示按系统定义顺序执行主流程。
...
| ----- | ------- | ------------------------------------------------------------------------------ |
| id    | PK      |                                                                                |
| key   | varchar | 如 ai_timeout_seconds、ai_dispatch_threshold、ai_review_threshold、welink_enabled；应用层拦截重复 |
| value | jsonb   | 配置值                                                                         |
...
> 生产 RBAC 不依赖 `User.role` 单字段。用户通过 Django `auth_group` 绑定一个或多个角色，通过角色获得权限点；`contact_id` 只用于接口人身份和数据范围绑定。v1 demo 可继续用前端演示身份，但不作为正式数据库口径。
...
> 数据范围不单独建表：v1 由角色权限 + `User.contact_id` + `Contact.department_id` 推导。二级接口人仅可见 `contact_id` 指向自己的 `AssignmentAttempt`，并可转派给所属二级部门下的三级接口人；三级接口人仅可见 `sub_contact_id` 指向自己的 `AssignmentAttempt`，并提交通过/未通过反馈。

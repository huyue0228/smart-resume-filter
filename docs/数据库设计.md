...
> `archive_reason` 存枚举，`archive_detail` 存人类可读说明和具体上下文，简历库按这两个字段展示归档原因。AI 建议归档、低于复核阈值、简历缺失/解析失败或最终失败且不生成 `AssignmentAttempt` 时，流程写入 `status=archived`、`archive_reason=agent_no_recommendation`，对应 `AgentDispatchDecision` 保留供 HR 查看和后续处置。AI 在当前投递未配置匹配启用岗位需求、已匹配岗位未配置有效二级部门、或已匹配岗位和二级部门但没有启用二级接口人时，也必须保留失败决策后归档，不调用模型。AI 开关仍开启且当前连接测试有效时可重试；如需 Rule，先关闭全局 AI 开关再发起新处理；手动分配始终可用。只有 Rule 当前志愿已匹配岗位和二级部门但缺启用二级接口人时，才使用正式阻塞码 `block_reason=contact_not_found` / `block_detail`，流程保持 `in_progress`，不写入归档原因。
...
> 状态约束：反馈一旦提交即锁定，不允许二次修改。`dispatched_l2` 仅允许绑定二级接口人在拥有 `attempt.view_received + attempt.feedback` 时于转派前反馈；`assigned_l3` 仅允许绑定三级接口人在拥有 `attempt.view_assigned + attempt.feedback` 时反馈，转派后二级接口人不可再提交。反馈「通过」后，`CandidateWorkflow.status=passed` 并取消同候选人其他未反馈尝试；反馈「未通过」后系统自动寻找下一志愿，若无可分配投递则归档。Rule 策略创建 `source=rule` 尝试，AI 策略创建 `source=ai` 尝试，HR 手动强制分配创建 `source=manual` 尝试。Rule 自动分配会校验投递招聘主体与岗位需求主体一致性，使用稳定岗位命中优先级选择 `resume_id` 匹配到的 `job_id`，再使用 `MajorCategory` / `MajorAlias` 对 `JobMajor` 需求专业和候选人最高学历专业做专业大类匹配；若大类无交集，或候选人/岗位任一侧未映射到启用大类，则再执行专业名双向包含兜底。岗位没有需求专业，或需求专业为通配表达时不阻断；`match_reason` 保存院校、志愿、岗位、专业大类/专业名、部门和接口人的匹配理由。手动强制分配绕过院校规则、志愿顺序和自动专业筛选；已通过候选人不可再通过手动强制分配入口分配，如需重开必须走简历库“处理简历”。反馈操作权限不扩大接口人数据范围。
...
> `AgentDispatchDecision` 只保存 AI 推荐动作、置信度、分项评分、理由、证据、风险点和失败分类等决策结果；AI 阈值、失败处理和状态流转以后端设计为准。AI 仅评估当前有效志愿从可能匹配的启用岗位中按招聘主体、岗位名命中级别和 `job.id` 兜底排序稳定选出的单一目标 `Job`；该目标不是数据库唯一约束。`evaluated_job_id` 记录该目标岗位，成功建议的 `recommended_job_id` 必须与其相同，不允许记录模型改荐的其它岗位；模型上下文只包含选定岗位的 `JobMajor`、二级部门和有效二级接口人。当前投递未配置匹配启用岗位需求、已匹配岗位未配置有效二级部门、或该部门没有启用二级接口人时均不调用模型，仍写失败决策并归档；岗位缺失决策的两个岗位外键可为空，后两类保留 `evaluated_job_id`。失败态最小必填为 `workflow_id`、`resume_id`、`processing_run_id`、`error_code`、`error_message` 和版本字段，不要求伪造推荐动作、置信度或理由。`error_code/error_message` 必须为服务端统一映射的稳定错误码和脱敏摘要，绝不保存第三方 SDK 原始异常。写入 `AgentDispatchDecision` 前，应用层对模型结构化输出中的字符串递归移除 NUL（`0x00`）字符，覆盖决策文本字段及 JSONField 内任意层级的字符串；AI 服务通过 `AIServiceError` 传递的受控错误消息在构造时执行相同清洗。除 NUL 外不改变其它字符，避免 PostgreSQL text/jsonb 拒绝写入；该处理不新增字段或迁移，也不表示对系统内所有数据库文本字段做全局清洗。表名保留 Agent，是因为 AI 策略内部以智能体方式完成简历阅读、当前岗位适配和分流建议。`resume_id + evaluated_job_id + file_checksum + profile_version + prompt_version + decision_version` 可用于应用层复用决策，其中 `file_checksum/profile_version` 通过 `profile_id` 关联 `ResumeProfile` 校验；不依赖数据库唯一约束。
...
- 简历库“处理简历”支持互斥的两种范围：`system_statuses + candidate_filters` 与当前表头、任务结果筛选相交；冻结的当前选中范围只提交非空正整数 `candidate_ids + force_reprocess=true`，不叠加状态或筛选。`force_reprocess` 只允许用于 Step2，表示 HR 主动重开：清空 `passed_attempt_id`、归档/阻塞原因及说明和完成时间，将流程恢复为 `in_progress`。提交后 revision 改变时安全跳过；普通导入的 `candidate_ids + source=resume_import` 不强制重开终态。AI 开启但连接未通过当前配置测试时拒绝，不降级为 Rule。
...
| status                   | varchar   | pending / running / waiting_conflict（兼容父运行状态；当前候选人冲突主要写 ScopeItem）/ cancelling / cancelled / success / partial_failed / failed / undone |
...
| archive_count            | int       | 建议归档或人工处理数量；AI 任务非零时可跳转对应任务筛选       |
...
> 步骤映射：`step1`=查重与志愿排序，`step2`=简历分类、分配与下发，`step3`=院校分类，`step4`=需求数据准备核对。`all` 按 `step3 → step4 → step1 → step2` 执行。上传含候选人时读取 `ai_enabled` 并只创建一条当前模式运行，按 `Step1 → Step2` 编排，绝不复用 `all`。简历库“处理简历”创建一条当前模式、只含 `step2` 的运行，范围为 `system_statuses + candidate_filters` 或互斥的 `candidate_ids + force_reprocess=true`；公开请求不接受 `modes`。生产异步提交返回 202，本地 eager 模式同步完成后返回 200，两者均返回 `processing_runs`。提交时将候选人和 `ProcessingRun.mode` 固化，后续切换开关不改变该任务。任务中心继续轮询运行及阶段记录并支持取消。
>
...
>
> 任务中心结果不新增持久化筛选字段。Rule / AI 主结果行的非零 `success` / `failed` / `skipped` / `cancelled` 均可跳转 `processing_run_id + processing_result` 的候选人列表，并按本运行 ScopeItem 终态筛选（`skipped` 对应 `skipped_manual_change`）。AI 的成功项可独立展开 `review` / `dispatch` / `archive`，按本运行 `AgentDispatchDecision.recommendation` 筛选；三个子项是成功的业务细分，始终展示但零值禁用，合计允许小于成功总数。
...
| total_count / processed_count / success_count / failed_count / skipped_count / cancelled_count | int | 阶段工作量及结果统计 |
...
| status | varchar | pending / queued / processing / waiting_conflict / success / failed / skipped_manual_change / cancelled |
...
> `candidate_id` 不可为空；`workflow_revision_at_submit=null` 只表示提交时该候选人尚无 `CandidateWorkflow`。ScopeItem 只为候选人处理阶段创建；Step3/Step4 等非候选人阶段不创建 ScopeItem。`(processing_run_id, candidate_id)` 数据库唯一约束防止同运行重复投递候选人。AI 任务以 ScopeItem 为权威执行状态：调度器用 `queued + dispatch_token` 保证发布幂等，候选人冲突时记为 `waiting_conflict` 并延时重试；Rule / AI 遇到人工变更导致令牌或 revision 失效时都记为 `skipped_manual_change`，不覆盖人工结果。任务结果筛选的 `success` / `failed` / `skipped` / `cancelled` 分别只读取本表的 `success` / `failed` / `skipped_manual_change` / `cancelled` 终态；待复核、待下发、建议归档不从 ScopeItem 推导。

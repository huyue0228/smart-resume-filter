...
> 步骤映射：`step1`=查重与志愿排序，`step2`=简历分类、分配与下发，`step3`=院校分类，`step4`=需求数据准备核对。`all` 按 `step3 → step4 → step1 → step2` 执行。上传含候选人时固定创建 Rule 运行，且仅 AI 已启用时创建额外 AI 运行；两个 `ProcessingRun` 独立记录、顺序先 Rule 后 AI 执行，各自按 `Step1 → Step2` 编排，绝不复用 `all`。简历库“处理简历”按钮按非空 `modes` 数组为每种已选模式创建独立、只含 `step2` 的运行，并携带 `scope.system_statuses`；缺失、空数组或非数组均拒绝。生产异步提交返回 202，本地 `CELERY_TASK_ALWAYS_EAGER=True` 同步完成后返回 200，两者均只返回 `processing_runs`。提交时将命中的候选人写入 `ProcessingRunScopeItem`，后续执行只读取固化范围。顶栏任务 Badge 与点击后的非遮罩侧边 Drawer 轮询这些运行及阶段记录。父运行只汇总状态、阶段指针和最终业务结果计数；页面连续进度由各 `ProcessingRunStage` 的累计工作量计算。`pending` 收到取消请求后立即写为 `cancelled`；`running` / `waiting_conflict` 写为 `cancelling`，执行器在候选人或阶段边界停止、保留已落库结果并写入 `cancelled_at`。任务中心活跃查询包含 `cancelling`。

...
> 流程口径：一个候选人业务上只维护一个当前分配流程，`CandidateWorkflow.candidate_id` 由 `OneToOneField` 施加数据库唯一约束。AI 任务只在领取和提交的短事务内以 `select_for_update` 锁定 ScopeItem/Candidate/Workflow，模型网络调用不持有数据库锁。人工分配、下发、转派、反馈等写操作会清除在途租约并递增 `revision`；AI 提交时令牌或版本不一致则安全跳过。归档候选人不单独建表，通过 `status=archived` + 归档字段查询；后续规则调整不会自动恢复归档流程，HR 可手动强制分配，或在简历库“处理简历”入口按弹窗选择的简历状态范围主动重开。
...
>
> AI 任务中心的成功、失败、待复核、待下发、归档、安全跳过、取消计数不新增持久化筛选字段。仅计数非零时可跳转 `processing_run_id + processing_result` 的候选人列表：`success` / `failed` / `skipped` / `cancelled` 按本运行 ScopeItem 终态（`skipped` 对应 `skipped_manual_change`）；`review` / `dispatch` / `archive` 按本运行 `AgentDispatchDecision.recommendation`，是 `success` 的可重叠业务细分。
...
> `candidate_id` 不可为空；`workflow_revision_at_submit=null` 只表示提交时该候选人尚无 `CandidateWorkflow`。ScopeItem 只为候选人处理阶段创建；Step3/Step4 等非候选人阶段不创建 ScopeItem。`(processing_run_id, candidate_id)` 数据库唯一约束防止同运行重复投递候选人。AI 任务以 ScopeItem 为权威执行状态：调度器用 `queued + dispatch_token` 保证发布幂等，候选人冲突时记为 `waiting_conflict` 并延时重试，人工变更导致令牌或 revision 失效时记为 `skipped_manual_change`，不提交在途 AI 结果。任务结果筛选的 `success` / `failed` / `skipped` / `cancelled` 分别只读取本表的 `success` / `failed` / `skipped_manual_change` / `cancelled` 终态；待复核、待下发、归档不从 ScopeItem 推导。
